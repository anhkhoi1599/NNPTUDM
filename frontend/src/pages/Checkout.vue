        ...
        <div class="checkout-form-container">
            <form id="checkoutForm" @submit="handleSubmit" novalidate autocomplete="off">
                <div class="checkout-heading">
                    <h3>Vài Bước Nữa Để Đặt Hàng<span>Tổng Cộng</span></h3>
                    <h3 v-if="user">{{ user.user_name }}'s Đơn Hàng<span>${{ calculateSummaryPrice()[3] }}</span></h3>
                </div>
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <input type="text" name="coAddress" id="coAddress" placeholder="Địa chỉ tại Sài Gòn, Vietnam"
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            ...
                            <input type="radio" name="payment" value="card" id="paymentCard"
                                v-model="checkoutObj.paymentMethod" /><span>Thẻ (Visa)</span>
                        </div>
        ...
        checkForm: function () {
            ...
            if (!this.checkoutObj.phone) {
                ...
                if (!this.checkoutObj.phone.startsWith('84')) {
                    this.errorObj.phoneErr.push('Số điện thoại bắt đầu với 84');
                }
                ...
                if (!/[0-9]{11}/.test(this.checkoutObj.phone)) {
                    this.errorObj.phoneErr.push('không chứa kí tự');
                }
            }

            // Address validate
            if (!this.checkoutObj.address) {
                this.errorObj.addressErr.push('Vui lòng nhập địa chỉ');
            }
            ...
            if (!this.checkoutObj.paymentMethod) {
                ...
                if (!this.cardObj.number) {
                    this.errorObj.numErr.push('Vui lòng nhập số thẻ');
                }
                else {
                    if (!this.cardObj.number.startsWith('4')) {
                        this.errorObj.numErr.push('Thẻ bắt đầu bằng 4');
                    }

                    if (this.cardObj.number.length != 16) {
                        this.errorObj.numErr.push('Thẻ là 16 chữ số');
                    }

                    if (!/[0-9]{16}/.test(this.cardObj.number)) {
                        this.errorObj.numErr.push('Số thẻ không chứa kí tự');
                    }
                }
                ...
                if (!this.cardObj.expiryDate) {
                    this.errorObj.exDateErr.push('Nhập ngày hết hạn');
                }


                if (!this.cardObj.cvv) {
                    this.errorObj.cvvErr.push('Nhập mã cvv');
                }
                else {
