        ...
        <div class="row">
            <div class="col-sm-4 col-12 filter-box">
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            ...
                            <label for="bsStatus" class="d-flex justify-content-between">Bán Chạy nhất
                        ...
                        <li>
                            <input type="button" name="cbStatus" id="ooStatus" value="Online Only" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="ooStatus" class="d-flex justify-content-between">Bán Online <button
                        ...
                        <li>
                            <input type="button" name="cbStatus" id="soStatus" value="Sale Off" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="soStatus" class="d-flex justify-content-between">Giảm giá <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                        ...
                        <li>
                            <input type="button" name="cbStatus" id="sdStatus" value="Seasonal Dishes" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="sdStatus" class="d-flex justify-content-between">Món ăn theo mùa <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                        ...
                        <li>
                            <input type="button" name="cbStatus" id="ndStatus" value="New Dishes" hidden
                                @click="filterStatusBtn($event)" />
                            <label for="ndStatus" class="d-flex justify-content-between">Món mới <button
                                    class="unselect-btn" @click="unselectStatusBtn($event)"
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            <input type="button" name="rPrice" id="rtfPrice" value="10000,15000" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rtfPrice" class="d-flex justify-content-between">10k - 15k <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rftPrice" value="16000,25000" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rftPrice" class="d-flex justify-content-between">16k - 25k <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="rttPrice" value="26000,35000" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="rttPrice" class="d-flex justify-content-between">26k - 35k <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="mtPrice" value="35000" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="mtPrice" class="d-flex justify-content-between">{{ ">" }} 35k <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rPrice" id="ltPrice" value="10000" hidden
                                @click="filterPriceBtn($event)" />
                            <label for="ltPrice" class="d-flex justify-content-between">{{ "<" }} 10k <button
                                    class="unselect-btn" @click="unselectPriceBtn($event)">X</button></label>
                        </li>
                ...
                <div class="row filter-section">
                    <ul class="filter-option">
                        <li>
                            <input type="button" name="rType" id="mType" value="Mặn" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="mType" class="d-flex justify-content-between">Mặn<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>

                        <li>
                            <input type="button" name="rType" id="vType" value="Chay" hidden
                                @click="filterTypeBtn($event)" />
                            <label for="vType" class="d-flex justify-content-between">Chay<button class="unselect-btn"
                                    @click="unselectTypeBtn($event)">X</button></label>
                        </li>
            ...
            <div class="col-sm-8">
                ...
                <div class="row box-container">
                    <div v-for="(f, index) in currentPageItems" :key="index">
                        <div class="box">
                            ...
                            <div class="content">
                                ...
                                <div class="price">
                                    {{ parseFloat(f.food_price) - parseFloat(f.food_discount) }}VND
                                    <span v-if="parseFloat(f.food_discount) != 0.00">{{ parseFloat(f.food_price)
                                    }}VND</span>
                                </div>
                                <button class="btn" @click="addItem(index)">Thêm Vào Giỏ</button>
                            </div>
                        </div>
                    </div>
                    <div v-if="!filterFoods.length">
                        <div class="box">
                            <div class="content">
                                <h1 style="color: #057835fa;">Không tìm thấy kết quả phù hợp!</h1>
                            </div>
                            <div class="image">
...
export default {
    ...
    methods: {
        ...
        evaluatePrice: function (food, priceRange) {
            ...
            else if (priceRange == "16000,25000") {
                if (16000 <= cal && cal <= 25000) {
                    return food;
                }
            }
            else if (priceRange == "26000,35000") {
                if (26000 <= cal && cal <= 35000) {
                    return food;
                }
            }
