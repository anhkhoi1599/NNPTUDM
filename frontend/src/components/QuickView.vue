<template>
        ...
        <div class="quick-view-inner" v-for="f in selectedFood" :key="f">
            ...
            <div class="product-detail d-flex">
                ...
                <div class="content">
                    ...
                    <p class="money">{{ parseFloat(f.food_price) - parseFloat(f.food_discount) }}VND<span
                            v-if="parseFloat(f.food_discount) > 0">${{
                                    parseFloat(f.food_price)
                            }}</span></p>
                    <div class="qty">
                        <label for="qty">Số lượng:</label>
                        <!-- gioi han so luong dat mon -->
    ...
    <div v-else class="quick-view">
        <div class="quick-view-inner">
            <h2 class="d-flex justify-content-between">Đăng nhập để tiếp tục
                <slot></slot>
            </h2>
            <div class="link-to-login" style="text-align: center; margin-top: 120px;">
                <router-link class="btn" to="/login" style="padding: 28px; font-size: 24px">Đăng nhập ngay
                </router-link>
            </div>
        </div>
    </div>
    ...
    methods: {
        ...
        async addToCart() {
            ...
            if (existItem.data.length == 1) {
                ...
                this.$refs.alert.showAlert('Thành công', 'Cảm ơn!', 'Đã thêm vào giỏ hàng thành công !')
